...
### 2. 忘记密码 / 重置密码流程（已完成）✅
- ~~邮箱发送重置链接~~ ✅（控制台log，开发模式）
- ~~新密码设置页面~~ ✅（/reset-password）
- ~~Token 过期处理（24h）~~ ✅（从1h改为24h）

### 3. Agent 截图/Logo 上传（已完成）✅
- ~~支持拖拽/点击上传图片~~ ✅
- ~~前端预览~~ ✅（ScreenshotsUpload组件）
- ~~后端存储（本地文件系统）~~ ✅
- ~~限制：最多 5 张截图，单张 ≤ 2MB，jpg/png/webp~~ ✅
...
### 5. Agent 详情页增强（已完成）✅
- ~~添加「相关 Agent 推荐」模块~~ ✅
- ~~浏览量统计展示~~ ✅（已存在，后端每次请求递增）

## 优先级排序
1. ~~UI 质感提升 + 移动端适配~~（已完成：Hero增强 + 卡片效果）
2. ~~安全加固~~（已完成：XSS防护 + 速率限制）
3. ~~错误处理统一~~（已完成：401/403/429处理）
4. ~~Agent 截图上传~~（已完成）
5. ~~忘记密码流程~~（已完成）
6. ~~用户动态 Feed~~（已完成）
7. ~~Agent 详情页增强~~（已完成：相关Agent推荐）
